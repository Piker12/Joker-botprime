...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '22664529962@s.whatsapp.net'; // ton numéro ou le groupe cible
...
╔══════✮❁•°♛°•❁✮ ═════╗
      ...
      *PIKER-BOTPRIME Bot Connected Successfully* ╚══════✮❁•°❀°•❁✮══════╝

> "Always Forward. PIKER-PRIME, one of the best."
╙┈♔◦☓◦☙◦♔◦☙◦☓◦♔┈╜
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 22664529962; // mettez votre numéro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '22664529962' };
                    configmanager.saveP();
                ...
                const code = await sock.requestPairingCode(number, 'PIKERGOT');
